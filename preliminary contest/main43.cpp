...
public:
    ...
    void sort_hash(){
        ...
        sort(hashes.begin(), hashes.end(), [&](int m, int m1){
            return map[m].key < map[m1].key;
        });
...
struct Route{
    void push_back(uint32_t index, uint32_t money, uint32_t money1 , uint32_t node, uint32_t node1){
        uint8_t& sz = size[index];
        if(sz < 16){
            route[index][sz].m = money;
            route[index][sz].m1 = money1;
            route[index][sz].n = node;
            route[index][sz].n1 = node1;
        }else if(sz == 16){
            memcpy(route1[index], route[index], 16 * sizeof(Record));
            route1[index][sz].m = money;
            route1[index][sz].m1 = money1;
            route1[index][sz].n = node;
            route1[index][sz].n1 = node1;
        }else{
            route1[index][sz].m = money;
            route1[index][sz].m1 = money1;
            route1[index][sz].n = node;
    ...
    Record route[maxn][16];
    Record route1[maxn][128];
    uint8_t size[maxn];
    bool sorted[maxn];
};

inline bool check(const uint32_t& m, const uint32_t& m1){
    return (m1 * 5UL >= m) && (m * 3UL >= m1);
}
...
/*
    Thread: search loops by dfs method.
*/
#ifdef DEBUG
    // Timer t;
#endif
    ...
    memset(mroute->size, 0, (ID.size() + 1) * sizeof(uint8_t));
    memset(mroute->sorted, false, (ID.size() + 1) * sizeof(bool));
    ...
    while(1){
        ...
        for(int j = qhead[node]; j < qhead[node + 1]; ++j){
            ...
            for(int k = qhead[node1]; k < qhead[node1 + 1]; ++k){
                ...
                for(int l = qhead[node2]; l < qhead[node2 + 1]; ++l){
                    ...
                    if(node3 <= node) break;
                    money3 = from3.money;
                    if(!check(money3, money2) || node3 == node ||  node3 == node1) continue;
                    mroute->push_back(node3, money3, money1, node2, node1);
        ...
        for(; j < phead[node + 1]; ++j){
            ...
            for(; k < phead[node1 + 1]; ++k){
                ...
                if(mroute->size[node2]){ // 五环
                    if(mroute->size[node2] <= 16){
                        if(!mroute->sorted[node2] && mroute->size[node2] > 1){
                            sort(mroute->route[node2], mroute->route[node2] + mroute->size[node2], [](const Record& lhs, const Record& rhs){
                                if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                return lhs.n < rhs.n;
                            });
                            mroute->sorted[node2] = true;
                        }
                        for(int x = 0; x < mroute->size[node2]; ++x){
                            money3 = mroute->route[node2][x].m;
                            money = mroute->route[node2][x].m1;
                            if(check(money2, money3) && check(money, money1)){
                                node3 = mroute->route[node2][x].n;
                                node4 = mroute->route[node2][x].n1;
                                if(node3 == node1) continue;
                                if(node4 == node1 || node4 == node2) continue;
                        ...
                        if(!mroute->sorted[node2]){
                            sort(mroute->route1[node2], mroute->route1[node2] + mroute->size[node2], [](const Record& lhs, const Record& rhs){
                                if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                return lhs.n < rhs.n;
                            });
                            mroute->sorted[node2] = true;
                        ...
                        for(int x = 0; x < mroute->size[node2]; ++x){
                            money3 = mroute->route1[node2][x].m;
                            money = mroute->route1[node2][x].m1;
                            if(check(money2, money3) && check(money, money1)){
                                node3 = mroute->route1[node2][x].n;
                                node4 = mroute->route1[node2][x].n1;
                                if(node3 == node1) continue;
                                if(node4 == node1 || node4 == node2) continue;
                ...
                for(; l < phead[node2 + 1]; ++l){
                    ...
                    node3 = to3.id;
                    money3 = to3.money;
                    if(!check(money2, money3)) continue; // || node3 == node1) continue;
                    if(node3 == node){
                        if(check(money3, money1)){ // 三环
                    ...
                    if(node3 == node1) continue;
                    if(mroute->size[node3]){
                        if(mroute->size[node3] <= 16){
                            if(!mroute->sorted[node3] && mroute->size[node3] > 1){
                                sort(mroute->route[node3], mroute->route[node3] + mroute->size[node3], [](const Record& lhs, const Record& rhs){
                                    if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                    return lhs.n < rhs.n;
                                });
                                mroute->sorted[node3] = true;
                            }
                            for(int x = 0; x < mroute->size[node3]; ++x){
                                money4 = mroute->route[node3][x].m;
                                money = mroute->route[node3][x].m1;
                                if(check(money3, money4) && check(money, money1)){
                                    node4 = mroute->route[node3][x].n;
                                    node5 = mroute->route[node3][x].n1;
                                    if(node4 == node1 || node5 == node1 || node4 == node2 || node5 == node2 || node5 == node3) continue;
                            ...
                            if(!mroute->sorted[node3]){
                                sort(mroute->route1[node3], mroute->route1[node3] + mroute->size[node3], [](const Record& lhs, const Record& rhs){
                                    if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                    return lhs.n < rhs.n;
                                });
                                mroute->sorted[node3] = true;
                            ...
                            for(int x = 0; x < mroute->size[node3]; ++x){
                                money4 = mroute->route1[node3][x].m;
                                money = mroute->route1[node3][x].m1;
                                if(check(money3, money4) && check(money, money1)){
                                    node4 = mroute->route1[node3][x].n;
                                    node5 = mroute->route1[node3][x].n1;
                                    if(node4 == node1 || node5 == node1 || node4 == node2 || node5 == node2 || node5 == node3) continue;
                                    loop6[0] = node;loop6[1] = node1;loop6[2] = node2;
                                    loop6[3] = node3;loop6[4] = node4;loop6[5] = node5;
                                    loop6 += 6;
                    ...
                    int m = phead[node3];
                    for(; m < phead[node3 + 1]; ++m) if(To[m].id >= node) break;
                    for(; m < phead[node3 + 1]; ++m){
                        const Edge& to4 = To[m];
                        node4 = to4.id;
                        money4 = to4.money;
                        if(!check(money3, money4)) continue;
                        if(node4 == node){
                            if(check(money4, money1)){ // 四环
                        ...
                        if(node4 == node1 || node4 == node2) continue;
                        if(mroute->size[node4]){ // 七环
                            if(mroute->size[node4] <= 16){
                                if(!mroute->sorted[node4] && mroute->size[node4] > 1){
                                    sort(mroute->route[node4], mroute->route[node4] + mroute->size[node4], [](const Record& lhs, const Record& rhs){
                                        if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                        return lhs.n < rhs.n;
                                    });
                                    mroute->sorted[node4] = true;
                                }
                                for(int x = 0; x < mroute->size[node4]; ++x){
                                    money5 = mroute->route[node4][x].m;
                                    money = mroute->route[node4][x].m1;
                                    if(check(money4, money5) && check(money, money1)){
                                        node5 = mroute->route[node4][x].n;
                                        node6 = mroute->route[node4][x].n1;
                                        if(node5 == node1 || node5 == node2 || node5 == node3) continue;
                                ...
                                if(!mroute->sorted[node4]){
                                    sort(mroute->route1[node4], mroute->route1[node4] + mroute->size[node4], [](const Record& lhs, const Record& rhs){
                                        if(lhs.n == rhs.n) return lhs.n1 < rhs.n1;
                                        return lhs.n < rhs.n;
                                    });
                                    mroute->sorted[node4] = true;
                                ...
                                for(int x = 0; x < mroute->size[node4]; ++x){
                                    money5 = mroute->route1[node4][x].m;
                                    money = mroute->route1[node4][x].m1;
                                    if(check(money4, money5) && check(money, money1)){
                                        node5 = mroute->route1[node4][x].n;
                                        node6 = mroute->route1[node4][x].n1;
                                        if(node5 == node1 || node5 == node2 || node5 == node3) continue;
                                        if(node6 == node1 || node6 == node2 || node6 == node3 || node6 == node4) continue;
                                        loop7[0] = node;loop7[1] = node1;loop7[2] = node2;loop7[3] = node3;
                                        loop7[4] = node4;loop7[5] = node5;loop7[6] = node6;
        ...
        for(int j = 0; j < reach.size(); ++j){
            mroute->size[reach[j]] = 0;
            mroute->sorted[reach[j]] = false;
        } 
...
void* sort_thread(void* arg){
    ...
    int tid = *(int*)arg;
    for(int i = tid; i < ID.size(); i += nthread){
        sort(To + phead[i], To + phead[i] + plen[i], [](const Edge& m, const Edge& m1){
            return m.id < m1.id;
        });
        sort(From + qhead[i], From + qhead[i] + qlen[i], [](const Edge& m, const Edge& m1){
            return m.id > m1.id;
        });
    }
}
